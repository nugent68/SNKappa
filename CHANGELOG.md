...
## 2026-08-02 — DESI DR1 comparison restored with permission (dual anchor)

S. Heydenreich / the DESI Lensing WG granted use of the v1.5 Lensing
Without Borders measurements ahead of their public release. Validation
(v) is now a two-tier closure: the fully public BOSS × KiDS-Legacy
anchor (below) PLUS the quantitative DESI DR1 comparison — fiducial
chain closes to 0–11% for BGS (z 0.1–0.4, extending the validated mass
range to median log M* = 10.4) and 12–21% for LRG (0.4–0.6; the
0.6–0.8 bin is high by 43%, satellite-dominated). Tracked artifact:
`desi_closure_summary.json` (amplitude ratios only — the raw measured
vectors remain untracked; permission to use and quote is not
redistribution). Generated by `scripts/desi_closure_summary.py` from
the local `notes/desi_internal/` copies of the 2026-07-10 NERSC runs.
